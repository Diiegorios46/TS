...
export function promisifyAll<T extends Record<string, (callback: (response: ApiResponse<any>) => void) => void>>(
    api: T
): { [K in keyof T]: () => Promise<any> } {
    const promisifiedApi = {} as { [K in keyof T]: () => Promise<any> };
    
    for (const key in api) {
        if (api.hasOwnProperty(key)) {
            promisifiedApi[key] = promisify(api[key]);
        }
    }
